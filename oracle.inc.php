...
if (isset($_GET["oracle2"])) {
        ...
        class Min_DB {
            ...
            function __construct() {
                ...
                foreach ($adminer->plugins as $plugin) {
                    if (get_class($plugin) == 'AdminerOracle2Driver') {
                        return;
                    }
                }
                adminer()->plugins[] = new AdminerOracle2Driver(true);
            }
            ...
            function connect($server, $username, $password) {
                ...
                $this->_offset_fetch = false;
                if (preg_match('/Release (\d+\.\d+)/i', $this->server_info, $matches)) {
                    $version = floatval($matches[1]);
                    if ($version >= 12.1) { // >= 12c
                        $this->_offset_fetch = true;
                    }
                }
                global $oracle_init_sqls;
    ...
    function get_databases() {
        return array(DUMMY_DATABASE);
    }

    function limit($query, $where, $limit, $offset = 0, $separator = " ") {
        $connection = connection();
        if ($connection->_offset_fetch) {
            return " $query$where OFFSET $offset ROWS FETCH FIRST $limit ROWS ONLY";
        } else {
    ...
    function tables_list() {
        ...
        $owner = where_owner("");
        return get_key_vals("SELECT table_name, 'table' FROM all_tables WHERE $owner
...
ORDER BY 1"
    ...
    function count_tables($databases) {
        ...
        foreach ($databases as $db) {
            $return[$db] = $connection->result("SELECT COUNT(*) FROM all_tables WHERE $owner");
        }
        return $return;
    }
    ...
    function table_status($name = "") {
        ...
        $view = views_table("view_name");
        $owner = where_owner("");
        foreach (get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows", tablespace_name "TableSpace" FROM all_tables WHERE ' . $owner . ($name != "" ? " AND table_name = $search" : "") . "
UNION SELECT view_name, 'view', 0, 0, '' \"TableSpace\" FROM $view" . ($name != "" ? " WHERE view_name = $search" : "") . "
ORDER BY 1"
        ) as $row) {
            if ($name != "") {
                return $row;
...
AND c_src.TABLE_NAME = " . q($table);
    ...
    function schemas() {
        $owner_in = "SELECT username FROM dba_users WHERE default_tablespace NOT IN ('SYSTEM','SYSAUX')";
        $return = get_vals("SELECT DISTINCT owner FROM dba_segments WHERE owner IN ($owner_in) ORDER BY 1");
        return ($return ? $return : get_vals("SELECT DISTINCT owner FROM all_tables WHERE owner IN ($owner_in) ORDER BY 1"));
    }
    ...
    function create_sql($table, $auto_increment, $style) {
        return implode("\n", get_vals("SELECT DBMS_METADATA.GET_DDL('TABLE','$table') FROM DUAL")) . "\n";
    }
...
');
    ...
    function support($feature) {
        return preg_match('~^(columns|database|drop_col|dump|indexes|descidx|processlist|scheme|sql|status|table|variables|view)$~', $feature); //!
    }
...
document.addEventListener('DOMContentLoaded', function() {
  var p = document.querySelector('.footer fieldset p');
  p.childNodes.forEach(function(node) {
    var text = node.textContent;
    if (text !== undefined && text.match(/<?php echo lang('Move to other database'); ?>/)) {
        node.textContent = text.replace(/<?php echo lang('Database');?>/, '<?php echo lang('Schema');?>');
    }
  });

  var elm = document.querySelector('#<?php echo $id; ?>');
  if (elm) {
...
<?php
      $table_spaces = array();
      foreach ($tables as $table => $status) {
          $table_spaces[] = "'$table': '{$status['TableSpace']}'";
      }
      echo 'var table_spaces = {' . implode(', ', $table_spaces) . '};';
?>
      table.querySelectorAll('tbody tr').forEach(function(tr) {
          var a = tr.querySelector('td:last-child').querySelector('a');
          if (!a) {
              return;
          }
          var name = a.id.split('-')[1];
          var td = document.createElement('td');
          td.append(table_spaces[name]);
          tr.append(td);
      });
  }
});
